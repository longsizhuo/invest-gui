    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="border border-[var(--border-subtle)] bg-[var(--surface-raised)] p-4">
          <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-2">5 个 verdict 选项</h3>
        ...
        <div className="border border-[var(--border-subtle)] bg-[var(--surface-raised)] p-4">
          <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-2">CIO Sanity Check（防 LLM 幻觉）</h3>
          <ul className="text-xs text-[var(--text-primary)] space-y-1">
      ...
      <div className="border border-[var(--border-subtle)] bg-[var(--surface-raised)] p-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <h4 className="text-xs text-[var(--text-secondary)] mb-2">阈值表</h4>
            <table className="w-full text-xs tabular-nums">
              <tbody>
                {Object.entries(data.regime_thresholds).map(([k, v]) => (
                  <tr key={k} className="border-t border-[var(--border-subtle)]">
                    <td className="px-2 py-1 font-mono text-[var(--text-secondary)]">{k}</td>
                    <td className="px-2 py-1 text-right text-[var(--accent)]">{v}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div>
            <h4 className="text-xs text-[var(--text-secondary)] mb-2">优先级 + 类型</h4>
            <ol className="text-xs text-[var(--text-primary)] space-y-1">
      ...
      <div className="space-y-4">
        <h3 className="text-sm font-semibold text-[var(--text-primary)]">🧠 4 个 Agent 角色（system prompt 全文）</h3>
      ...
      <div className="border border-[var(--border-subtle)] bg-[var(--surface-raised)] p-4">
        ...
        <div className="space-y-2">
          {data.tools.map((tool, i) => {
            ...
            return (
              <div key={i} className="border-t border-[var(--border-subtle)] pt-2">
                <code className="text-[var(--accent)] font-mono text-sm">{fn?.name ?? "?"}</code>
                <p className="text-xs text-[var(--text-secondary)] mt-1">{fn?.description ?? ""}</p>
              </div>
            );
          })}
        </div>
  ...
  return (
    <div className="border border-[var(--border-subtle)] bg-[var(--surface-raised)] p-4">
      ...
      {(agent.notes ?? []).length > 0 && (
        <div className="mb-3">
          <h4 className="text-xs font-semibold text-[var(--text-primary)] mb-1">硬规则</h4>
...
function PromptDetails({ label, prompt }: { label: string; prompt: string }) {
  return (
    <details className="mt-2 rounded bg-[var(--surface-base)]/60 border border-[var(--border-subtle)] p-2">
      <summary className="text-xs text-[var(--text-secondary)] cursor-pointer hover:text-[var(--text-primary)]">
        📄 {label}（{prompt.length} 字）
      </summary>
      <pre className="mt-2 text-xs text-[var(--text-primary)] whitespace-pre-wrap max-h-96 overflow-auto">
        {prompt}
      </pre>
    </details>
  );
